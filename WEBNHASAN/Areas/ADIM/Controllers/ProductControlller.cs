...
namespace WEBNHASAN.Controllers
{
    public class ProductControlller : Controller
    {
        ...
        public IActionResult Index(int? page, string timkiemten="")
        {
            var pageIndex = (int)(page != null ? page : 1);
            var pageSize = 6;
            var productList = _db.Products.Include(x => x.Category).Where(p=>p.Name.ToLower().Contains(timkiemten.ToLower())).ToList();
